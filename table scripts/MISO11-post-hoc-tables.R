...
save <- function(tbl, name){
  write.table(tbl, file=here(paste("tables/post-hoc-supplementary/", name, ".csv", sep="")), sep=",", col.names=F, row.names=F)
  print(xtable(tbl), type="html", file=here(paste("tables/post-hoc-supplementary/", name, ".html", sep="")))
}
